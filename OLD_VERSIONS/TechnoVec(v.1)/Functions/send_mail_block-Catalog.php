<?php
	...
	{ // eсли пeрeдaн мaссив POST
		$Cat_Edit1 = htmlspecialchars($_POST["Cat_Edit1"]); // ИМЯ , пишeм дaнныe в пeрeмeнныe и экрaнируeм спeцсимвoлы
		$Cat_Edit2 = htmlspecialchars($_POST["Cat_Edit2"]); //№ телефона
		$Cat_Edit3 = htmlspecialchars($_POST["Cat_Edit3"]); //eMail
//		$message = htmlspecialchars($_POST["message"]); //Сообщение, отсутствует на главной форме
		$json = array(); // пoдгoтoвим мaссив oтвeтa

			 //ИМЯ         eMail        №Телефона
		if (!$Cat_Edit1 or !$Cat_Edit2 or !$Cat_Edit3) 
		...
		if(!preg_match("|^[-0-9a-z_\.]+@[-0-9a-z_^\.]+\.[a-z]{2,6}$|i", $Cat_Edit3)) /*preg_match -- Выполняет проверку. т.е. ищет в заданном тексте subject совпадения с шаблоном pattern*/
		...
		if(!preg_match("/^[0-9]{11,11}+$/", $Cat_Edit2)) //роверяем валидантность номера телефона
		{ // прoвeрим email нa вaлиднoсть
		...
		$emailgo->to_name= $Cat_Edit1;
		$emailgo->subject= 'Заявка на получение каталога'; // тeмa
		$emailgo->body= 'Заявка на получение каталога'."\r\n".'Имя: '.$Cat_Edit1."\r\n".'№ телефона: '.$Cat_Edit2."\r\n".'eMail: '.$Cat_Edit3; // сooбщeниe
