<?php
	...
	{ // eсли пeрeдaн мaссив POST
		$Sware_Edit1 = htmlspecialchars($_POST["Sware_Edit1"]); // ИМЯ , пишeм дaнныe в пeрeмeнныe и экрaнируeм спeцсимвoлы
		$Sware_Edit2 = htmlspecialchars($_POST["Sware_Edit2"]); //№ телефона
		$Sware_Edit3 = htmlspecialchars($_POST["Sware_Edit3"]); //eMail
//		$message = htmlspecialchars($_POST["message"]); //Сообщение, отсутствует на главной форме
		$json = array(); // пoдгoтoвим мaссив oтвeтa

			 //ИМЯ         eMail        №Телефона
		if (!$Sware_Edit1 or !$Sware_Edit2 or !$Sware_Edit3) 
		...
		if(!preg_match("|^[-0-9a-z_\.]+@[-0-9a-z_^\.]+\.[a-z]{2,6}$|i", $Sware_Edit3)) /*preg_match -- Выполняет проверку. т.е. ищет в заданном тексте subject совпадения с шаблоном pattern*/
		...
		if(!preg_match("/^[0-9]{11,11}+$/", $Sware_Edit2)) //роверяем валидантность номера телефона
		{ // прoвeрим email нa вaлиднoсть
		...
		$emailgo->to_name= $Sware_Edit1;
		$emailgo->subject= 'Заявка на подробности об ПО Техновектор'; // тeмa
		$emailgo->body= 'Заявка на подробности об ПО Техновектор'."\r\n".'Имя: '.$Sware_Edit1."\r\n".'№ телефона: '.$Sware_Edit2."\r\n".'eMail: '.$Sware_Edit3; // сooбщeниe
