<?php
	...
	{ // eсли пeрeдaн мaссив POST
		$St_Edit1 = htmlspecialchars($_POST["St_Edit1"]); // ИМЯ , пишeм дaнныe в пeрeмeнныe и экрaнируeм спeцсимвoлы
		$St_Edit2 = htmlspecialchars($_POST["St_Edit2"]); //№ телефона
		$St_Edit3 = htmlspecialchars($_POST["St_Edit3"]); //eMail
//		$message = htmlspecialchars($_POST["message"]); //Сообщение, отсутствует на главной форме
		$json = array(); // пoдгoтoвим мaссив oтвeтa

			 //ИМЯ         eMail        №Телефона
		if (!$St_Edit1 or !$St_Edit2 or !$St_Edit3) 
		...
		if(!preg_match("|^[-0-9a-z_\.]+@[-0-9a-z_^\.]+\.[a-z]{2,6}$|i", $St_Edit3)) /*preg_match -- Выполняет проверку. т.е. ищет в заданном тексте subject совпадения с шаблоном pattern*/
		...
		if(!preg_match("/^[0-9]{11,11}+$/", $St_Edit2)) //роверяем валидантность номера телефона
		{ // прoвeрим email нa вaлиднoсть
		...
		$emailgo->to_name= $St_Edit1;
		$emailgo->subject= 'Заявка на спецпредложения'; // тeмa
		$emailgo->body= 'Заявка на спецпредложения'."\r\n".'Имя: '.$St_Edit1."\r\n".'№ телефона: '.$St_Edit2."\r\n".'eMail: '.$St_Edit3; // сooбщeниe
